...
class fun(commands.Cog):
    def __init__(self, client):
        self.client = client
        self.reddit = praw.Reddit(client_id=config.REDDIT_ID, client_secret=config.REDDIT_S, user_agent='A discord bot | https://github.com/Hitsounds/discord-bot')
        os.mkdir("temp/ping/")
    ...
    async def ping_(self, ctx):
        ...
        uimg = Image.open(await lib.file_from_url(ctx.message.author.avatar_url_as(static_format="png", size=tempdDetails["rq_size"])))
        background = Image.open(await lib.file_from_url(f"https://www.hitsounds.moe/hitcircles/assets/ping/{template}"))
        mask = Image.new('L', size, 0)
        ImageDraw.Draw(mask).ellipse((0, 0) + size, fill=255)
        background.paste(uimg, (tempdDetails["x"], tempdDetails["y"]), mask)
        background.save(f"temp/ping/ping_out_{ctx.message.author.name}.png")
        with open(f"temp/ping/ping_out_{ctx.message.author.name}.png", "rb") as f:
            await lib.sendfile(self, f, d_ctx=ctx)
        os.remove(f"temp/ping/ping_out_{ctx.message.author.name}.png")
